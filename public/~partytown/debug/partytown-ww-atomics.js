...
((self) => {
    ...
    const getConstructorName = (obj) => {
        ...
        try {
            const zoneJsConstructorName =
                null === (_c = null === (_b = null == obj ? void 0 : obj.__zone_symbol__originalInstance) || void 0 === _b ? void 0 : _b.constructor) ||
                void 0 === _c
                    ? void 0
                    : _c.name;
    ...
    const getOrCreateSerializedInstance = ([winId, instanceId, nodeName]) =>
        instanceId === winId && environments[winId] ? environments[winId].$window$ : getOrCreateNodeInstance(winId, instanceId, nodeName);
        ...
        ((target, applyPath, args, rtnValue) => {
            if (webWorkerCtx.$config$.logCalls) {
                try {
                    ...
                    logWorker(
                        `Call ${getTargetProp(target, applyPath)}(${args.map((v) => getLogValue(applyPath, v)).join(', ')}), returned: ${getLogValue(
                            applyPath,
                            rtnValue
                        )}`,
                        target[WinIdKey]
        ...
        scriptContent =
            `with(this){${
                (webWorkerCtx.$config$.globalFns || [])
                    ...
                    .join(';') + scriptContent.replace(/\bthis\b/g, '(thi$(this)?window:this)').replace(/\/\/# so/g, '//Xso')
                        ...
                        $window$: new Proxy(win, {
                            get: (win, propName) => {
                                ...
                                if ('string' != typeof propName || isNaN(propName)) {
                                    return (null === (_a = webWorkerCtx.$config$.mainWindowAccessors) || void 0 === _a ? void 0 : _a.includes(propName))
