            ...
            if (scriptElm) {
                ...
                if (!winCtx.$isInitialized$) {
                        ...
                        forwards.map((forwardProps) => {
                            ...
                            forwardProps.split('.').map((_, i, arr) => {
                                mainForwardFn = mainForwardFn[arr[i]] =
                                    i + 1 < len(arr) ? mainForwardFn[arr[i]] || ('push' === arr[i + 1] ? [] : {}) : (...args) => forwardCall(arr, args);
                            });
                        });
                    ...
                    doc.dispatchEvent(new CustomEvent('pt0'));
                    {
                        ...
                        logMain(
                            `Executed ${winType} window ${normalizedWinId($winId$)} environment scripts in ${(performance.now() - winCtx.$startTime$).toFixed(
                                1
                            )}ms`
                        );
                    }
        ...
        try {
            if (isValidMemberName(memberName) && isNaN(memberName[0]) && 'all' !== memberName) {
                ...
                if ('function' === memberType) {
                    ...
                    'symbol' !== memberType &&
                        (memberName.toUpperCase() === memberName ? interfaceMembers.push([memberName, 6, value]) : interfaceMembers.push([memberName, 6]));
                }
            }
        } catch (e) {
            console.warn(e);
